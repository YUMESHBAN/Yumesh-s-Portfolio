...
import { ArrowLeft, ArrowRight, ArrowUpRight, ChevronDown, ChevronUp, Code2, FileText, Github, Play, Sparkles } from "lucide-react";
...
export function ProjectCaseStudy({ project, index, total, nextProject, relatedContent }: CaseStudyProps) {
  ...
  const hasBuildNotes = Boolean(project.techStack.length || project.features.length || project.demoVideoUrl || project.projectPdfUrl || project.links?.length || project.additionalDocuments?.length || gallery.length);
