...
export default async function ArticlesPage() {
  ...
  const topFeatured = articles.filter((article) => article.featuredOnArchive);
  const remainingArticles = articles.filter((article) => !article.featuredOnArchive);

  return (
    <section className="articles-page site-section pt-20 sm:pt-24">
      <div className="site-container">
        ...
        {topFeatured.length > 0 || remainingArticles.length > 0 ? (
          <>
            {topFeatured.length ? (
              <div className="articles-featured-list mt-10 sm:mt-12">
                {topFeatured.map((article, index) => (
                  <article
                    key={article.slug}
                    className={`articles-feature border-b border-white/10 ${index === 0 ? "border-t" : ""}`}
                  >
                    <Link
                      href={`/articles/${article.slug}`}
                      className="articles-feature-link block px-4 py-8 sm:px-6 sm:py-9 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300/70"
                    >
                      <div className="articles-feature-grid grid gap-x-5 gap-y-5 sm:grid-cols-[3.5rem_1.5rem_minmax(0,1fr)_auto_1rem] sm:items-start sm:gap-x-7">
                        <p className="articles-feature-number font-mono text-base tracking-[0.16em] text-blue-300">
                        ...
                        <div className="articles-feature-content">
                          <div className="articles-feature-meta flex flex-wrap items-center gap-x-3 gap-y-1 font-mono text-[10px] uppercase tracking-[0.14em] text-white/55">
                            <span className="articles-feature-category text-blue-200/80">
                              {index === 0 ? "Latest article" : "Featured article"}
                            </span>
                          ...
                          <p className="articles-feature-excerpt mt-3 max-w-2xl text-sm leading-6 text-white/62 sm:text-base sm:leading-7">
                            {article.excerpt}
                          </p>
                        </div>
            ...
            {remainingArticles.length ? (
              <section className="articles-index mt-12 sm:mt-16" aria-labelledby="articles-index-title">
                <div className="articles-index-heading flex items-center justify-between gap-4 border-b border-white/10 px-4 pb-4 sm:px-6">
                  <h2 id="articles-index-title" className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/55">
                    {topFeatured.length > 0 ? "More writing" : "All writing"}
                  </h2>
                ...
                <div className="articles-index-list">
                  {remainingArticles.map((article, index) => (
                    <article key={article.slug} className="articles-row border-b border-white/10">
                      <Link href={`/articles/${article.slug}`} className="articles-row-link grid gap-x-5 gap-y-4 px-4 py-7 sm:px-6 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300/70 sm:grid-cols-[3.5rem_1.5rem_minmax(0,1fr)_auto_1rem] sm:items-start sm:gap-x-7">
                        <p className="articles-row-number font-mono text-sm tracking-[0.16em] text-blue-300">{String(index + topFeatured.length + 1).padStart(2, "0")}</p>
