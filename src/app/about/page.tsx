            ...
            <div className="about-proof-shelf mt-12 border-y border-white/10 py-2 sm:py-3">
              {certifications.map((certification, index) => (
                <article key={`${certification.issuer}-${certification.title}`} tabIndex={certification.credentialUrl ? undefined : 0} aria-label={`${certification.title}, ${certification.issuer}`} className={`about-proof-shelf-item group relative border-b border-white/10 py-5 last:border-b-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-300/70 sm:py-6 ${certification.credentialUrl ? "cursor-pointer" : ""} ${index % 2 ? "lg:ml-10" : ""}`}>
                  ...
                  <div className="about-proof-shelf-detail grid grid-cols-1 gap-5 border border-blue-300/45 px-5 sm:grid-cols-[10rem_minmax(0,1fr)] sm:items-start sm:gap-8 sm:px-7">
                    ...
                    <div>{certification.description ? <p className="text-sm leading-7 text-white/62">{certification.description}</p> : null}{certification.issueDate || certification.expiryDate || certification.credentialId ? <dl className="mt-5 grid gap-4 border-t border-white/10 pt-4 text-sm sm:grid-cols-3"><div>{certification.issueDate ? <><dt className="font-mono text-[10px] uppercase tracking-[0.14em] text-blue-200/70">Issued</dt><dd className="mt-1.5 text-white/70">{certification.issueDate}</dd></> : null}</div><div>{certification.expiryDate ? <><dt className="font-mono text-[10px] uppercase tracking-[0.14em] text-blue-200/70">Expires</dt><dd className="mt-1.5 text-white/70">{certification.expiryDate}</dd></> : null}</div><div>{certification.credentialId ? <><dt className="font-mono text-[10px] uppercase tracking-[0.14em] text-blue-200/70">Credential ID</dt><dd className="mt-1.5 break-all font-mono text-xs text-white/70">{certification.credentialId}</dd></> : null}</div></dl> : null}{certification.relatedSkills?.length ? <div className="mt-5 border-t border-white/10 pt-4"><p className="font-mono text-[10px] uppercase tracking-[0.14em] text-blue-200/70">Related skills</p><div className="mt-3 flex flex-wrap gap-2">{certification.relatedSkills.map((skill) => <span key={skill._id ?? skill.name} className="site-chip">{skill.name}</span>)}</div></div> : null}</div>
                  </div>
                </article>
              ))}
            </div>
